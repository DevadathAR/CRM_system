...
class CustomDateTimePicker extends StatefulWidget {
  ...
  const CustomDateTimePicker({
    ...
    this.initialDate,
    this.onDateSelected, this.hintText,
  }) : super(key: key);
...
class _CustomDateTimePickerState extends State<CustomDateTimePicker> {
  ...
  void initState() {
    ...
    _textController = TextEditingController(
      text: widget.iconName == calenderSvg
          ? (_selectedDate != null ? _formatDateTime(_selectedDate!, _selectedTime) : '')
 ...
 String _formatTime(TimeOfDay time) {
    ...
    final String hourString = hour == 0 ? '12' : (hour < 10 ? '0$hour' : '$hour');
    final String minuteString = time.minute < 10 ? '0${time.minute}' : '${time.minute}';
    ...
    if (selectedDate != null) {
      setState(() {
        ...
        _textController.text = widget.iconName == calenderSvg
            ? _formatDateTime(selectedDate, _selectedTime) // Update text field with date and time
            : _formatTime(_selectedTime ?? TimeOfDay.now()); // Only show time if icon is not calendar
      });
  ...
  Future<void> _pickTime(BuildContext context) async {
    ...
    if (selectedTime != null) {
      setState(() {
        ...
        _textController.text = widget.iconName == calenderSvg
            ? _formatDateTime(_selectedDate ?? DateTime.now(), selectedTime) // Update text field with date and time
        ...
        decoration: InputDecoration(
          ...
          suffixIcon: IconButton(
            ...
            onPressed: widget.iconName == calenderSvg
                ? () => _pickDate(context) // Handle calendar icon
                : () => _pickTime(context), // Handle time picker icon
          ),
          hintText:widget.hintText,
          hintStyle: AppTextStyle.regularText(size: 14, color: AppColors.textGrey1),
        ),
      ),
    );
  }
